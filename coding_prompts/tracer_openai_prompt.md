...
- All AI hardcoded AI prompts are to live hardcoded in the client side code in lib/prompts/ as .ts files.  
    Files and their contents:
        - openaiSystemPrompt.ts
            - openaiSystemPrompt(user_name: string): string
...
- POST /api/tracer/session
    - Body: { userId: 1, sdp: "<offer>" }
    - Server: fetch user name -> build prompt -> build tools -> POST /v1/realtime/client_secrets -> POST /v1/realtime/calls -> return SDP answer
    - Response: { date: { sdp: "<answer" }, error: null}
...
Error handling: connection failure shows error banner, SDP failure shows error, mid-conversation drop show banner
